...
The full release check builds wheel and sdist, runs metadata checks, rejects forbidden archive contents/credentials, installs
the exact wheel in a clean environment, verifies import/CLI/uv, exercises the private uv bootstrap, runs live and
prepared-offline smoke tests, bootstraps a second environment from the exact bundled wheel set with `--no-index`, and
prints exact artifact hashes for manual publication. CI repeats the appropriate gates by operating system, architecture,
